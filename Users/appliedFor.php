    ...
    <div class="">
        ...
        <div class="container-fluid">
            ...
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                ...
                <li class="nav-item">
                <a class="nav-link active" href="appliedFor.php">Applied For</a>
                </li>
            </ul>
        ...
        <div class="p-5">
            ...
            <table id="appliedForTable" class="display" style="width:100%;">
        ...
        <!-- Modal -->
        <div class="modal fade" id="editJobApplication" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            ...
            <div class="modal-footer">
                ...
                <input type="hidden" id="apply_job_id">
            </div>
            </div>
        </div>
        </div>
...
<script>
    ...
    function readData(){
        let read = true;
        $.post('../php/applyForController.php',{
            read:read
        },function(data,status){
            $('#appliedForTable').html(data);
            new DataTable('#appliedForTable');
        });
    }
    
    function jobLoc($str,uid,jid){
        let jobL=true;
        let user_id = uid;
        let job_id=jid;
        let str = $str;
    ...
    function examCenter($str,uid,jid){
        let examC=true;
        let user_id = uid;
        let job_id=jid;
        let str = $str;
    ...
    function editApplication($uid,$jid){

        $('#apply_job_id').val($jid);
        let edit='edit';
        let uid=$uid;
        let jid=$jid;
        $.post('../php/applyForController.php',{
            ...
            jobLoc(res.job_location,uid,jid);
            examCenter(res.exam_center,uid,jid);
            $('#height').val(res.height);
            $('#highSchoolPercentage').val(res.highschool_percentage);
            $('#intermediatePercentage').val(res.intermediate_percentage);
    ...
    function applyChanges(){
        ...
        let jobid = $('#apply_job_id').val();
        let height = $('#height').val();
        let highschool_percentage =$('#highSchoolPercentage').val();
        let intermediate_percentage =$('#intermediatePercentage').val();
        let prefered_job_location =$('#preferedJobLocation').val();
        let prefered_exam_center =$('#preferedExamCenter').val();
        ...
        $.post('../php/applyForController.php',{
            ...
            prefered_exam_center:prefered_exam_center,
            address:address

        },function (data,status){
            if(data == 'updated'){
                alert('Job application updated successfully....');
                $('#editJobApplication').modal('hide');
                readData();
            }else{
